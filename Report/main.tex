...
\textbf{Mẫu Sử dụng Bộ định thời}
...
Trình điều khiển cảm biến sử dụng bộ định thời bắt tín hiệu đầu vào để đo thời gian phản hồi của xung siêu âm. Quá trình đo bao gồm kích hoạt xung Trigger và đo độ rộng xung Echo sử dụng ngắt Input Capture.
...
Trạng thái này xử lý việc nhập tiền từ người dùng, cộng dồn số tiền đã trả và kiểm tra xem đã đủ để thanh toán chưa.

\begin{lstlisting}[language=C, caption={Xử lý Thanh toán}]
case PAYMENT_INPUT:
{
    if (timeout_timer_flag == 1) {
        // Timeout handling...
...
Chế độ quản trị cho phép người dùng có thẩm quyền thay đổi thông tin sản phẩm.
...
\begin{lstlisting}[language=C, caption={Các hàm Xác thực Đầu vào}]
// Quantity validation
...
\begin{lstlisting}[language=C, caption={Giảm số lần cập nhật hiển thị}]
...
Chỉ sử dụng một ngắt SysTick 1ms duy nhất cho tất cả bộ định thời:
...
\textbf{Tất cả các đường dẫn lỗi được kiểm thử thành công:}
